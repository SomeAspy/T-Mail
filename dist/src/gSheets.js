...
export async function append2sheet(readings) {
    const parsedData = [];
    config.googleSheets.data.forEach((item) => {
        parsedData.push(fillBlanks(item, readings));
    });
    const jwtClient = new google.auth.JWT(gAuth.client_email, undefined, gAuth.private_key, ['https://www.googleapis.com/auth/spreadsheets']);
